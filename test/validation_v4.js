...
const validate = async () => {
  ...
  // load the outputs
  const checkBody = fs.readFileSync('ihelp_check_v4.csv', 'utf8');
  ...
  const getOutputs = async (input) => {
    ...
    const charitypoolPASS = withinError(parseFloat(charitypool.toFixed(2)), result['Cumulative Charity Pool DAI']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const devpoolPASS = withinError(parseFloat(devpool.toFixed(2)), result['Cumulative Dev Pool DAI']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const stakepoolPASS = withinError(parseFloat(stakepool.toFixed(2)), result['Cumulative Staking Pool DAI']) ? chalk.green('PASS') : chalk.red('FAIL');;
