...
const validate = async() => {
  ...
  // load the outputs
  const checkBody = fs.readFileSync('ihelp_check_v3.csv', 'utf8');

  const check = await csv().fromString(checkBody);

  const ERROR_THRESHOLD = 0.01;
  const withinError = (value, actual) => {
    if (actual == 0) {
      if (value == 0) {
        return true;
      }
  ...
  const getOutputs = async(input) => {
    ...
    const c1walletTx = await ihelp.getClaimableCharityInterestOf(charity1wallet);
    const c1wallet = fromBigNumber(c1walletTx);

    const c2walletTx = await ihelp.getClaimableCharityInterestOf(charity2wallet);
    const c2wallet = fromBigNumber(c2walletTx);

    const c3walletTx = await ihelp.getClaimableCharityInterestOf(charity3wallet);
    ...
    const c1contributionsPASS = withinError(parseFloat(c1contributions.toFixed(2)), result['Contribution Balance C1 DAI']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const c2contributionsPASS = withinError(parseFloat(c2contributions.toFixed(2)), result['Contribution Balance C2 USDC']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const c3contributionsPASS = withinError(parseFloat(c3contributions.toFixed(2)), result['Contribution Balance C3 DAI']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const phasePASS = withinError(phase, result['Phase']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const helpsupplyPASS = withinError(parseFloat(helpsupply.toFixed(2)), result['Total HELP Supply']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const helpcirculatingPASS = withinError(parseFloat(helpcirculating.toFixed(2)), result['Total HELP Circulating']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const helpunclaimedPASS = withinError(parseFloat(helpunclaimed.toFixed(2)), result['Unclaimed HELP Tokens All Users']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const c1interestPASS = withinError(parseFloat(c1interest.toFixed(2)), result['Total Interest Earned C1']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const c2interestPASS = withinError(parseFloat(c2interest.toFixed(2)), result['Total Interest Earned C2']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const c3interestPASS = withinError(parseFloat(c3interest.toFixed(2)), result['Total Interest Earned C3']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const totalinterestPASS = withinError(parseFloat(totalinterest.toFixed(2)), result['Cumulative Interest Earned USD']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const charitypoolPASS = withinError(parseFloat(charitypool.toFixed(2)), result['Charity Interest Pools']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const devpoolPASS = withinError(parseFloat(devpool.toFixed(2)), result['Development Pool']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const stakepoolPASS = withinError(parseFloat(stakepool.toFixed(2)), result['Staking Pool Balance']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const c1walletPASS = withinError(parseFloat(c1wallet.toFixed(2)), result['C1 Wallet Balance DAI']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const c2walletPASS = withinError(parseFloat(c2wallet.toFixed(2)), result['C2 Wallet Balance DAI']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const c3walletPASS = withinError(parseFloat(c3wallet.toFixed(2)), result['C3 Wallet Balance DAI']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const user1helpPASS = withinError(parseFloat(user1help.toFixed(2)), result['U1 HELP Balance']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const user2helpPASS = withinError(parseFloat(user2help.toFixed(2)), result['U2 HELP Balance']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const user1xhelpPASS = withinError(parseFloat(user1xhelp.toFixed(2)), result['U1 XHELP Balance']) ? chalk.green('PASS') : chalk.red('FAIL');;
    const user2xhelpPASS = withinError(parseFloat(user2xhelp.toFixed(2)), result['U2 XHELP Balance']) ? chalk.green('PASS') : chalk.red('FAIL');;
    ...
    console.log('   Charity Interest Pools', '\t\t', parseFloat(charitypool.toFixed(2)), '\t\t', result['Charity Interest Pools'], '\t\t', charitypoolPASS);
    console.log('   Development Pool', '\t\t\t', parseFloat(devpool.toFixed(2)), '\t\t', result['Development Pool'], '\t\t', devpoolPASS);
    console.log('   Staking Pool', '\t\t\t', parseFloat(stakepool.toFixed(2)), '\t\t', result['Staking Pool Balance'], '\t\t', stakepoolPASS);
  ...
  const approvalTx5u1 = await ihelp.connect(userSigner1).approve(xhelpAddress, web3.utils.toWei('50000'));
  await approvalTx5u1.wait();
  const stakeTx5u1 = await xhelp.connect(userSigner1).deposit(web3.utils.toWei('50000'));
  await stakeTx5u1.wait();
